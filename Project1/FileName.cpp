...
int main() {
    ...
    Image image;
    image.loadFromFile("tryng.png");

    // Set the transparent color
    image.createMaskFromColor(sf::Color::Transparent);

    // Create a texture from the non-transparent parts of the image
    sf::Texture textingttt;
    textingttt.setRepeated(true);

    textingttt.loadFromImage(image);

    // Create a sprite from the texture
    Sprite floor(textingttt);
    floor.setPosition(Vector2f(0, 1110));
   // floor.setScale(0.65f, 0.65f);
    ...
    while (window.isOpen()) {
        ...
        // fireboy movments
        // fireboy jumping
        if ((!fireboy.getGlobalBounds().intersects(floor.getGlobalBounds()))) {
        ...
        // to the up
        if ((Keyboard::isKeyPressed(Keyboard::Key::Up)) && (fireboy.getGlobalBounds().intersects(floor.getGlobalBounds()))) 
        {
        ...
        //fireboy right and left movements
        {
            // to the right
        ...
        // watergirl movments(right and left)
        {
            // to the right
        ...
        // watergirl jumping
        if ((!watergirl.getGlobalBounds().intersects(floor.getGlobalBounds())) ) 
        {
        ...
        if (Keyboard::isKeyPressed(Keyboard::Key::W) && ((watergirl.getGlobalBounds().intersects(floor.getGlobalBounds())))) 
        {
