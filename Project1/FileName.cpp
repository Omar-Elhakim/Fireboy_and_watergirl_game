...
int main() {
    // Main window
    RenderWindow window(VideoMode(1210, 850), "Fireboy and Watergirl");
    Image icon;
...
bool doesIntersect(Sprite& player, RectangleShape ground[]) {
    bool state = false;
    for (int i = 0; i < 29; i++) {
        if ((player.getGlobalBounds().intersects(ground[i].getGlobalBounds())) && (!player.getGlobalBounds().intersects(ground[4].getGlobalBounds()))) {
            state = true;
        }
    }
    return state;
